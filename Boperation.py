import sys
from BAExcept import WithdrawError,InsuffFundError,DepositError,InvalidAccountError
import random
import cx_Oracle
...
def OpenAccount():
    ...
    if Amount<500:
              ...
              print("="*60)
              bcur.execute("insert into bank values('%s','%s','%s',%d,'%s',%d)"%(Name,DOB,Address,Amount,AccountNumber,Mobile))
              acur.execute("update atm set Amount=%d where AccountNumber='%s'"%(Amount,AccountNumber))
              con.commit()
              print("\tWelcome to '{}' in my Bank Application".format(Name))
              print("\tYour A/C is Open Successfully")
              print("="*60)
    bcur.execute("select* from bank where AccountNumber='%s'"%AccountNumber)
    for data in bcur:
            print("\tName  :\t\t",data[0])
            print("\tDOB  : \t\t",data[1])
            print("\tMobile No.:\t",data[5])
...
def Withdraw():
    ...
    bcur.execute("update bank set Amount=%d where AccountNumber='%s'"%(totm,ac))
    acur.execute("update atm set Amount=%d where AccountNumber='%s'"%(totm,ac))
    con.commit()
    ...
    for data in bcur:
        if(ac==data[4]):
            ...
            if(omn==data[5]):
                ...
                print("******************* Thanku **********************")
                bcur.execute("update bank set Mobile=%d where AccountNumber='%s'"%(nmn,ac))
                acur.execute("update atm set Mobile=%d where AccountNumber='%s'"%(nmn,ac))
                con.commit()
                break
            else:
                print("="*60)
...
def CloseAccount():
    ...
    for data in bcur:
        if(ac==data[4]):
            ...
            print("\tA/C Number:\t",data[4])
            print("="*60)
            bcur.execute("delete  from bank  where AccountNumber='%s'"%ac)
            acur.execute("delete from atm where AccountNumber='%s'"%ac)
            con.commit()
            break
    else:
        raise InvalidAccountError
          ...
          for data1 in bcur:
                if(ac==data1[4]):                    
                          ...
                          if(ac==fac):
                                    ...
                                    break
                          bcur.execute("select* from bank where AccountNumber='%s'"%fac)
                          for data2 in bcur:
                                if(fac==data2[4]):
                                    ...
                                    break
                          bcur.execute("update bank set Amount=%d where AccountNumber='%s'"%(totm1,fac))
                          acur.execute("update atm set Amount=%d where AccountNumber='%s'"%(totm1,fac))
                          bcur.execute("update bank set Amount=%d where AccountNumber='%s'"%(totm2,ac))
                          acur.execute("update atm set Amount=%d where AccountNumber='%s'"%(totm2,ac))
                          con.commit()
                          break
